...
def plot_target_distribution(df: pd.DataFrame):
    ...
    df["Survived"].value_counts().plot(
        kind="bar", ax=ax[0], color=["#e74c3c", "#2ecc71"]
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "reports/figures/01_target_distribution.png", dpi=150, bbox_inches="tight"
    )
...
def plot_survival_by_class(df: pd.DataFrame):
    ...
    plt.legend(title="Survived", labels=["No", "Yes"])
    plt.savefig(
        "reports/figures/03_survival_by_pclass.png", dpi=150, bbox_inches="tight"
    )
...
def plot_age_distribution(df: pd.DataFrame):
    ...
    df[df["Survived"] == 0]["Age"].hist(
        bins=30, alpha=0.5, label="Not Survived", ax=ax[1], color="#e74c3c"
    )
    df[df["Survived"] == 1]["Age"].hist(
        bins=30, alpha=0.5, label="Survived", ax=ax[1], color="#2ecc71"
    )
...
def plot_correlation_heatmap(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(
        "reports/figures/05_correlation_heatmap.png", dpi=150, bbox_inches="tight"
    )
